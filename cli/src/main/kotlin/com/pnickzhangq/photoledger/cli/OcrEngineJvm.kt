...
class OcrEngineJvm(
    ...
    private fun detectBoxes(pixels: IntArray, width: Int, height: Int): List<List<List<Float>>> {
        ...
        for (y in 0 until rh32) {
            for (x in 0 until rw32) {
                ...
                val px = if (inContent) pixels[sy * width + sx] else 0
                val idx = y * rw32 + x
        ...
        OnnxTensor.createTensor(env, input, shape).use { tensor ->
            detSession.run(mapOf(detSession.inputNames.first() to tensor)).use { output ->
                ...
                return dbPostprocess(probMap, rw, rh, width, height)
            }
        }
    }
    ...
    private fun dbPostprocess(
        ...
        for (start in bin.indices) {
            ...
            val x0 = ((minX - unclip) * scaleX).coerceIn(0f, origW - 1f)
            val y0 = ((minY - unclip) * scaleY).coerceIn(0f, origH - 1f)
            val x1 = ((maxX + 1 + unclip) * scaleX).coerceIn(0f, origW - 1f)
            val y1 = ((maxY + 1 + unclip) * scaleY).coerceIn(0f, origH - 1f)
            boxes.add(listOf(
